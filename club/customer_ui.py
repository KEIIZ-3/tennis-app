...
<div class="ticket-notice" style="border-color:#60a5fa; background:#eff6ff; color:#1e3a8a;">
  <span class="ticket-notice-icon" style="background:#0b55b7;">!</span>
  <div>
    <p class="ticket-notice-title" style="color:#1e3a8a; margin-bottom:8px;">🎫 チケットについて</p>
    ...
    <p class="ticket-notice-title" style="color:#1e3a8a; margin-top:12px; margin-bottom:4px;">📅 ご予約について</p>
    <p class="ticket-notice-text">コート手配の都合上、レッスンのご予約は開催日の1週間前までにお願いいたします。</p>
  </div>
</div>
""".strip()

    old_notice_pattern = re.compile(
        r'<div class="ticket-notice">\s*'
        r'<span class="ticket-notice-icon">✓</span>\s*'
        r'<div>\s*'
        r'<p class="ticket-notice-title">チケットが足りない場合もご予約いただけます。</p>\s*'
        r'<p class="ticket-notice-text">\s*'
        r'通常レッスンは最後の1名になるとキャンセルできませんので、ご注意ください。\s*'
        r'</p>\s*'
        r'</div>\s*'
        r'</div>',
        re.DOTALL,
    )
    html = old_notice_pattern.sub(replacement_notice, html, count=1)
