...
def reservation_kind_admin(obj):
...
def cancellation_reason_admin(obj):
...
def cancellation_source_admin(obj):
    if getattr(obj, "status", "") == getattr(Reservation, "STATUS_RAIN_CANCELED", "rain_canceled"):
        return "雨天中止"
...
def apply_reservation_admin_history_columns():
    ...
    model_admin.reservation_kind_admin = reservation_kind_admin.__get__(
        model_admin,
        model_admin.__class__,
    )
    model_admin.cancellation_reason_admin = cancellation_reason_admin.__get__(
        model_admin,
        model_admin.__class__,
    )
    model_admin.canceled_at_admin = canceled_at_admin.__get__(
        model_admin,
        model_admin.__class__,
    )
    model_admin.cancellation_source_admin = cancellation_source_admin.__get__(
        model_admin,
        model_admin.__class__,
    )
