...
class CustomerReservationUiTests(TestCase):
    ...
    def test_ticket_page_uses_view_context_without_response_rewrite(self):
        ...
        self.assertContains(response, "保有中のチケット内訳")
        self.assertContains(response, "チケット購入予約")
        self.assertContains(response, "購入予約履歴")
        self.assertContains(response, "チケット履歴")
        self.assertNotContains(response, "チケットで確認すること")
        self.assertNotContains(response, "すぐ確認する")
        self.assertNotContains(response, "予約一覧へ")
        self.assertNotContains(response, ">予約する<", html=False)
        self.assertNotContains(response, ">予約確認<", html=False)
        self.assertNotContains(response, ">料金<", html=False)
        self.assertNotContains(response, ">保有内訳<", html=False)
        self.assertNotContains(response, ">予約分の差し引き<", html=False)
        self.assertNotContains(response, ">履歴<", html=False)
