...
class FixedLesson(models.Model, LessonTypeMixin):
    ...
    def sync_future_reservations(self, created_by=None):
        ...
        実装をモデル内に複製せず、管理画面・M2Mシグナルと同じ原子的な
        同期サービスへ委譲する。遅延importによりmodelsとの循環参照を避ける。
        """
        from .fixed_lesson_sync_facade import synchronize_fixed_lesson_membership

        return synchronize_fixed_lesson_membership(self.pk, created_by=created_by)
...
class Reservation(models.Model, LessonTypeMixin):
    ...
    def consume_tickets(self, reason="reservation_use", created_by=None, note=""):
        ...
        with transaction.atomic():
            ...
            purchase_remaining_total = sum(
                int(purchase.remaining_tickets) for purchase in purchases
            )
            unknown_legacy_tickets = max(
                int(locked_user.ticket_balance) - purchase_remaining_total,
                0,
            )
            remaining_to_consume = max(
                int(locked_self.tickets_used) - unknown_legacy_tickets,
                0,
            )
            ...
            if unknown_legacy_tickets == 0 and remaining_to_consume == 0:
