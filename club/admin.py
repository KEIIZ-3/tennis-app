...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        ("会員情報", {"fields": ("full_name", "phone_number", "email", "ticket_balance", "is_profile_completed")}),
...
class CoachAvailabilityAdmin(admin.ModelAdmin):
    ...
    list_display = ("id", "coach", "court", "lesson_type", "start_at", "end_at", "capacity")
    list_filter = ("coach", "court", "lesson_type")
...
class FixedLessonAdmin(admin.ModelAdmin):
    list_display = ("id", "title", "coach", "court", "lesson_type", "weekday", "start_hour", "capacity", "weeks_ahead", "is_active")
    list_filter = ("coach", "court", "lesson_type", "weekday", "is_active")
...
class ReservationAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status", "lesson_type", "coach", "court", "is_fixed_entry")
    search_fields = (
        "user__username",
        "user__full_name",
        "coach__username",
    ...
    actions = ("rain_cancel_selected_reservations",)

    @admin.action(description="選択した予約を雨天中止にして参加者へLINE送信する")
    def rain_cancel_selected_reservations(self, request, queryset):
        canceled_count = 0
        for reservation in queryset.select_related("user", "coach", "court"):
            try:
                changed = reservation.mark_rain_canceled(
                    created_by=request.user,
                    reason="雨天中止",
                ...
                self.message_user(request, f"{reservation} の雨天中止処理に失敗しました: {e}", level=messages.ERROR)
